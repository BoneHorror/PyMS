...
class PyAI(MainWindow):
	...
	def open(self, key=None, aiscript_data=None, aiscript_path=None, bwscript_data=None, bwscript_path=None): # type: (None, bytes, str, bytes, str) -> None
		if not self.unsaved():
			...
			self.ai = ai
			self.strings = {}
			for id,ai in list(self.ai.ais.items()):
	...
	def remove(self, key=None):
		...
		for index in indexs:
			...
			if e[0] in self.ai.externaljumps[e[1]][0]:
				for d in list(self.ai.externaljumps[e[1]][0][e[0]].items()):
					for id in d[1]:
		...
		if cantremove:
			...
			t = '\n'.join(['\t%s referenced by: %s' % (id,', '.join(refs)) for id,refs in list(cantremove.items())])
