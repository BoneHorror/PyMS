			...
			for id,dat in headers.items():
				for n,o in enumerate(dat[2]):
					if o != None:
						...
						load_offset(o, id)
			ocode = OrderedDict(sorted(iter(code.items()), key=lambda item: item[0]))
	...
	def interpret(self, files, offset=None, checkframes=None):
		...
		if unused:
			for l in unused:
				...
				r = OrderedDict(sorted(iter(code.items()), key=lambda item: item[0]))
		...
		k = list(c.keys())
		k.sort()
		self.code = OrderedDict(sorted(iter(c.items()), key=lambda item: item[0]))
